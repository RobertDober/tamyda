defmodule Tamyda.DataTest do
  ...
  describe "resource_tags" do
    ...
    def resource_tag_fixture(attrs \\ %{}) do
      {:ok, resource_tag} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Data.create_resource_tag()

      resource_tag
    end

    test "list_resource_tags/0 returns all resource_tags" do
      resource_tag = resource_tag_fixture()
      assert Data.list_resource_tags() == [resource_tag]
    end

    test "get_resource_tag!/1 returns the resource_tag with given id" do
      resource_tag = resource_tag_fixture()
      assert Data.get_resource_tag!(resource_tag.id) == resource_tag
    end

    test "create_resource_tag/1 with valid data creates a resource_tag" do
      assert {:ok, %ResourceTag{} = resource_tag} = Data.create_resource_tag(@valid_attrs)
    end

    test "create_resource_tag/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Data.create_resource_tag(@invalid_attrs)
    end

    test "update_resource_tag/2 with valid data updates the resource_tag" do
      resource_tag = resource_tag_fixture()
      assert {:ok, %ResourceTag{} = resource_tag} = Data.update_resource_tag(resource_tag, @update_attrs)
    end

    test "update_resource_tag/2 with invalid data returns error changeset" do
      resource_tag = resource_tag_fixture()
      assert {:error, %Ecto.Changeset{}} = Data.update_resource_tag(resource_tag, @invalid_attrs)
      assert resource_tag == Data.get_resource_tag!(resource_tag.id)
    end

    test "delete_resource_tag/1 deletes the resource_tag" do
      resource_tag = resource_tag_fixture()
      assert {:ok, %ResourceTag{}} = Data.delete_resource_tag(resource_tag)
      assert_raise Ecto.NoResultsError, fn -> Data.get_resource_tag!(resource_tag.id) end
    end

    test "change_resource_tag/1 returns a resource_tag changeset" do
      resource_tag = resource_tag_fixture()
      assert %Ecto.Changeset{} = Data.change_resource_tag(resource_tag)
    end
  end
end
